...
def lista():
    ...
    query = Ordem.query.join(Cliente)

    if busca:
        query = query.filter(
            db.or_(
                Cliente.nome.ilike(f'%{busca}%'),
                Ordem.dispositivo.ilike(f'%{busca}%'),
                Ordem.problema.ilike(f'%{busca}%')
            )
        )
    ...
    ordens = query.order_by(Ordem.criado_em.desc()).all()
    return render_template('ordens/lista.html', ordens=ordens, busca=busca, status=status)
...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        ordem.garantia_dias = int(request.form.get('garantia_dias') or 0)
